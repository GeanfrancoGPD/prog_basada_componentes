export default class ThemeSwitcher extends HTMLElement {
  ...
  async setTheme(name) {
    try {
      ...
      document.dispatchEvent(
        new CustomEvent("themeChanged", { detail: { themeName: name } }),
      );
      if (typeof this._onChange === "function") this._onChange(name);
    } catch (error) {
      slice.logger.logError(
        "ThemeSwitcher",
        `Could not switch to theme "${name}"`,
        error,
      );
    }
    this._sync(name);
  ...
  set themes(value) {
    this._themes =
      Array.isArray(value) && value.length ? value : ["Light", "Dark"];
    this._sync();
  }
  get themes() {
    return this._themes;
